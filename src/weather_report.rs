...
pub async fn get_weather_report(station: String) -> Result<WeatherReport, DwdError> {
    ...
    let url = format!("https://opendata.dwd.de/weather/weather_reports/poi/{}-BEOB.csv", station);
...
pub fn parse_weather_report<R: std::io::Read>(report: R) -> Result<WeatherReport, DwdError> {
    ...
    let properties: Vec<&str> = properties
        .iter()
        .skip(2)
        .collect();
    ...
    let data: Vec<HashMap<String, Value>> = iter.map(|record| {
        if record.is_err() {
            return None;
        }
        ...
        let timestamp = DateTime::<Utc>::from_utc(NaiveDateTime::parse_from_str(&format!("{} {}", date, time), "%d.%m.%y %H:%M").ok()?, Utc).timestamp_millis();
        ...
        for value in record {
            ...
            if !UNDEF_REGEX.is_match(value) {
                map.insert(prop, if let Ok(value) = f32::from_str(&value.replace(",", ".")) {
                    Value::from(value)
                } else {
                    Value::from(value)
                });
